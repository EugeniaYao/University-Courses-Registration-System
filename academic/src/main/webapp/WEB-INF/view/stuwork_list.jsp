						...
						<div class="card card-shadow mb-4">
							<div class="card-body">
								<table id="bs4-table" class="table table-bordered table-striped" cellspacing="0" width="100%">
									...
									<tbody>
									<c:forEach items="${stuworkList }" var="obj" varStatus="_v">
										<tr>
											<td>${_v.count }</td>
											<td>${obj.realname }</td>
											<td>${obj.title }</td>
											<td>
												<a href="<%=basePath%>file/${obj.filepath }">Download</a>
											</td>
											<td>${obj.addtime }</td>
											<td>
												<c:if test="${obj.scorce==0}">
													No Grade
												</c:if>
												<c:if test="${obj.scorce!=0}">
													${obj.scorce }
											...
											<td>
												<c:if test="${obj.scorce==0 && roler=='2'}">
													<a class="btn btn-warning btn-sm" href="<%=basePath%>stuwork/beforeAddOrUpdateStuwork?stuworkId=${obj.stuworkid}&homeworkId=${obj.homeworkid}">Edit</a>
													<button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#delStuWork" onclick="javascript:$('#delId').val('${obj.stuworkid}');">Delete</button>
												</c:if>
												<c:if test="${roler=='1'}">
													<button type="button" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#setScorce" onclick="javascript:$('#setId').val('${obj.stuworkid}');">Grade</button>
												</c:if>
											</td>
										</tr>
									</c:forEach>
...
<div id="delStuWork" class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<form action="<%=basePath%>stuwork/deleteStuwork" method="post">
				<input type="hidden" name="stuworkId" id="delId" value="">
...
<div id="setScorce" class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg">
		<div class="modal-content">
			<form action="<%=basePath%>stuwork/setScorce" method="post">
				<input type="hidden" name="stuworkId" id="setId" value="">
				<div class="modal-header">
				...
				<div class="modal-body">
					<div class="form-group">
						<label>Points</label>
						<input type="text" class="form-control form-control-pill" name="scorce" value="">
					</div>
				</div>
				<div class="modal-footer">
					<button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
